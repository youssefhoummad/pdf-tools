...
class AppGui(tk.Frame):
  ...
  def init_sidebar(self):
    ...
    menu = [
      {'title':SPLIT_TITLE,'icon':IMG_CUT, 'index':0},
      {'title':MERGE_TITLE,'icon':IMG_MERGE, 'index':1 },
      {'title':CROP_TITLE,'icon':IMG_CROP, 'index':2 },
      {'title':EXTRACT_TITLE,'icon': IMG_EXTRCT, 'index':3 },
      {'title':IMAGES_TITLE,'icon': IMG_IMAGES, 'index':4 },
      {'title':ABOUT_TITLE, 'icon': IMG_ABOUT, 'index':5}
      ]
    # show top <PDF-Tools>
    CardFrame(self.sidebar, 'PDF tools', MAIN_DESC, justify=START_DIR).pack()

    # add buttons sidebar to list <btns_sidebar>
    for item in menu:
      self.btns_sidebar.append(
        ItemSidebare(self.sidebar, item['title'], icon=item['icon'], justify=START_DIR, command=lambda x=item['index']:self.show_current_sceen(x)))
  ...
  def split_screen(self):
    ...
    header = ImageLabel(_f, SPLIT_HEADER,width=550, height=180)
    header.pack()

    CardFrame(_f, SPLIT_TITLE, SPLIT_DESC, justify=START_DIR).pack(padx=START_PADDING_24, fill='both', pady=(0, 10))

    BrowsePDF(_f, self.file_path, text=BROWSE, justify=START_DIR).pack(padx=START_PADDING_24, anchor=NW, fill='x', expand=True)
    ...
    GifLabel(_f, LOADING_GIF).pack(side=START_DIR, anchor='sw', pady=(0,2))
    return _f
  ...
  def merge_screen(self):
    ...
    header = ImageLabel(_f, MERGE_HEADER,width=550, height=180)
    header.pack()

    CardFrame(_f, MERGE_TITLE, MERGE_DESC, justify=START_DIR).pack(padx=START_PADDING_24, fill='both', pady=(0, 10))

    BrowsePDF(_f, self.file_path, text=BROWSE, justify=START_DIR).pack(padx=START_PADDING_24, anchor=NW, fill='x', expand=True)
    BrowsePDF(_f, self.file_path2, text=BROWSE, justify=START_DIR).pack(padx=START_PADDING_24, pady=(10,0), anchor=NW, fill='x', expand=True)
  ...
  def crop_screen(self):
    ...
    CardFrame(_f, CROP_TITLE, CROP_DESC, justify=START_DIR).pack(padx=START_PADDING_24, fill='both', pady=(0, 10))
  ...
  def extract_screen(self):
    ...
    header = ImageLabel(_f, EXTRACT_HEADER,width=550, height=180)
    header.pack()

    CardFrame(_f, EXTRACT_TITLE, EXTRACT_DESC, justify=START_DIR).pack(padx=START_PADDING_24, fill='both', pady=(0, 10))

    BrowsePDF(_f, self.file_path, text=BROWSE, justify=START_DIR).pack(padx=START_PADDING_24, anchor=NW, fill='x', expand=True)

    _c = tk.Frame(_f, bg=_f['bg'])
    Switch(_c, text=EACH_PAGE_TEXT, variable=self.to_single_pages, justify=START_DIR, bg=_f['bg']).pack(side=START_DIR, anchor=NW, fill='x', expand=True)
  ...
  def to_pdf_screen(self):
    ...
    header = ImageLabel(_f, CONVERT_HEADER ,width=550, height=180)
    header.pack()

    CardFrame(_f, IMAGES_TITLE, IMAGES_DESC, justify=START_DIR).pack(padx=START_PADDING_24, fill='both', pady=(0, 10))

    BrowseImage(_f, self.files_path, text=BROWSE, justify=START_DIR).pack(padx=START_PADDING_24, anchor=NW, fill='x', expand=True)
  ...
  def about_screen(self):
    ...
    CardFrame(_f, ABOUT_TITLE, ABOUT_DESC, justify=START_DIR).pack(padx=START_PADDING_24, fill='both', pady=(0, 0))
